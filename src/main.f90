...
program ROHSA
  ...
  real(xp) :: lambda_abs_amp_joint !! lambda for amplitude parameter
  real(xp) :: lambda_abs_mu_joint  !! lamnda for mean position parameter
  real(xp) :: lambda_abs_sig_joint !! lambda for dispersion parameter
  ...
  lambda_mu = 1._xp
  lambda_sig = 1._xp
  lambda_abs_amp = 1._xp
  lambda_abs_mu = 1._xp
  lambda_abs_sig = 1._xp
  lambda_abs_tot = 0._xp
  lambda_abs_amp_joint = 0._xp
  lambda_abs_mu_joint = 1._xp
  lambda_abs_sig_joint = 0._xp
  ...
  call read_parameters(filename_parameters, filename, filename_abs, fileout, filename_noise, filename_noise_abs, n_gauss, &
       lambda_amp, lambda_mu, lambda_sig, lambda_abs_amp, lambda_abs_mu, lambda_abs_sig, lambda_abs_tot, lambda_abs_amp_joint, &
       lambda_abs_mu_joint, lambda_abs_sig_joint, lambda_var_amp, lambda_var_mu, lambda_var_sig, amp_fact_init, sig_init, &
       amp_fact_init_abs, sig_init_abs, init_option, maxiter_init, maxiter, m, noise, regul, descent, lstd, ustd, iprint, &
  ...
  call main_rohsa(data, data_abs, std_cube, std_cube_abs, fileout, n_gauss, lambda_amp, lambda_mu, lambda_sig, &
       ...
       lambda_abs_sig_joint, lambda_var_amp, lambda_var_mu, lambda_var_sig, amp_fact_init, sig_init, amp_fact_init_abs, &
       sig_init_abs, maxiter_init, maxiter, m, noise, regul, descent, lstd, ustd, init_option, iprint, iprint_init, &
